...
void ThreadedBinaryTree::inorder_recursive(TBTree *node) {
    if (node == NULL) return;
    if (!node->lthread) inorder_recursive(node->left);
    cout << node->data << " ";
    if (!node->rthread) inorder_recursive(node->right);
}

void ThreadedBinaryTree::preorder_recursive(TBTree *node) {
    if (node == NULL) return;
    cout << node->data << " ";
    if (!node->lthread) preorder_recursive(node->left);
    if (!node->rthread) preorder_recursive(node->right);
}

void ThreadedBinaryTree::postorder_recursive(TBTree *node) {
    if (node == NULL) return;
    if (!node->lthread) postorder_recursive(node->left);
    if (!node->rthread) postorder_recursive(node->right);
    cout << node->data << " ";
}
...
void ThreadedBinaryTree::menu() {
    ...
    do {
        ...
        cin >> choice;
        switch (choice) {
            case 1: cout << "Enter value to insert: "; cin >> val; insert(val); break;
            case 2: inorder_recursive(root); cout << endl; break;
            case 3: preorder_recursive(root); cout << endl; break;
            case 4: postorder_recursive(root); cout << endl; break;
            case 5: inorder_nonrecursive(); cout << endl; break;
            case 6: preorder_nonrecursive(); cout << endl; break;
            case 7: cout << "Enter value to search: "; cin >> val; cout << (search(val) ? "Node found!\n" : "Node not found.\n"); break;
            case 8: cout << "Enter value to delete: "; cin >> val; deleteKey(val); break;
            case 9: return;
